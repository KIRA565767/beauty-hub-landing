import { productRoutes, routeRooms, terminalPanel } from "../../content/landing";
          ...
          {productRoutes.map((route) => {
            ...
            return (
              <a
                key={route.id}
                href={route.path}
                className="grid min-h-11 grid-cols-[76px_1fr] items-center gap-3 border-t border-dusty/10 py-3 text-left transition hover:text-copper focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-copper"
              >
                <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-copper">{room?.roomCode?.split(" /")[0]}</span>
