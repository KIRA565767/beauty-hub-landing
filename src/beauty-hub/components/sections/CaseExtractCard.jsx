...
export function CaseExtractCard({ item, index }) {
  ...
  const routeLabel = item.route ? String(item.route).toUpperCase() : "ROUTE";
  const details = [
    ...
    <Reveal delay={0.03 * index} className="group relative overflow-hidden private-pass p-5 md:p-6">
      ...
      <div className="flex items-start justify-between gap-4 border-b border-dusty/10 pb-5">
        <div className="min-w-0 pl-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-copper">case {number} / route</p>
          <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.16em] text-mineral">{routeLabel}</p>
        </div>
      ...
      <div className="mt-6 border-t border-dusty/10">
        {details.map((detail) => (
          <div
            key={detail.label}
            className="grid gap-3 border-b border-dusty/10 py-4 md:grid-cols-[132px_minmax(0,1fr)] md:gap-5"
          >
